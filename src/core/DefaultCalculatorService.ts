...
export class DefaultCalculatorService implements CalculatorService {
    ...
    setInput(data: string): CalculatorAction {
        return compound(state => DefaultCalculatorService.setNormalizedInput(this.inputNormalizer.normalizeInput(state, data)));
    }
    ...
    submitInput(): CalculatorAction {
        return compound(state => {
            ...
            return foldEither((errors: ErrorList) => DefaultCalculatorService.sendInputErrors(errors),
